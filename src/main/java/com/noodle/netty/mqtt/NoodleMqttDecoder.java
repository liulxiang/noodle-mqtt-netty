...
public class NoodleMqttDecoder extends ReplayingDecoder<NoodleMqttDecoder.DecoderState> {
    ...
    public NoodleMqttDecoder() {
        this(DEFAULT_MAX_BYTES_IN_MESSAGE);
    }

    public NoodleMqttDecoder(int maxBytesInMessage) {
        super(NoodleMqttDecoder.DecoderState.READ_FIXED_HEADER);
    ...
    protected void decode(ChannelHandlerContext ctx, ByteBuf buffer, List<Object> out) throws Exception {
        switch (state()) {
            case READ_FIXED_HEADER: try {
                ...
                checkpoint(NoodleMqttDecoder.DecoderState.READ_VARIABLE_HEADER);
                // fall through
            ...
            case READ_VARIABLE_HEADER:  try {
                final NoodleMqttDecoder.Result<?> decodedVariableHeader = decodeVariableHeader(ctx, buffer, mqttFixedHeader);
                variableHeader = decodedVariableHeader.value;
                if (bytesRemainingInVariablePart > maxBytesInMessage) {
                    throw new TooLongFrameException("too large message: " + bytesRemainingInVariablePart + " bytes");
                }
                bytesRemainingInVariablePart -= decodedVariableHeader.numberOfBytesConsumed;
                checkpoint(NoodleMqttDecoder.DecoderState.READ_PAYLOAD);
                // fall through
            } catch (Exception cause) {
                out.add(invalidMessage(cause));
                return;
            ...
            case READ_PAYLOAD: try {
                final NoodleMqttDecoder.Result<?> decodedPayload =
                        decodePayload(
                                ctx,
                                buffer,
                ...
                checkpoint(NoodleMqttDecoder.DecoderState.READ_FIXED_HEADER);
    ...
    private MqttMessage invalidMessage(Throwable cause) {
        checkpoint(NoodleMqttDecoder.DecoderState.BAD_MESSAGE);
    ...
    private static MqttFixedHeader decodeFixedHeader(ChannelHandlerContext ctx, ByteBuf buffer) {
        ...
        return NoodleMqttCodecUtil.validateFixedHeader(ctx, NoodleMqttCodecUtil.resetUnusedFields(decodedFixedHeader));
    }
     ...
     */
    private NoodleMqttDecoder.Result<?> decodeVariableHeader(ChannelHandlerContext ctx, ByteBuf buffer, MqttFixedHeader mqttFixedHeader) {
        switch (mqttFixedHeader.messageType()) {
            ...
            case PINGRESP:
                ...
                return new NoodleMqttDecoder.Result<Object>(null, 0);
            default:
                //shouldn't reach here
    ...
    private static NoodleMqttDecoder.Result<MqttConnectVariableHeader> decodeConnectionVariableHeader(
            ChannelHandlerContext ctx,
            ByteBuf buffer) {
        final NoodleMqttDecoder.Result<String> protoString = decodeString(buffer);
        ...
        NoodleMqttCodecUtil.setMqttVersion(ctx, version);
        ...
        if (version == MqttVersion.MQTT_5) {
            final NoodleMqttDecoder.Result<MqttProperties> propertiesResult = decodeProperties(buffer);
        ...
        final MqttConnectVariableHeader mqttConnectVariableHeader = new MqttConnectVariableHeader(
                ...
                keepAlive,
                properties);
        return new NoodleMqttDecoder.Result<MqttConnectVariableHeader>(mqttConnectVariableHeader, numberOfBytesConsumed);
    }

    private static NoodleMqttDecoder.Result<MqttConnAckVariableHeader> decodeConnAckVariableHeader(
            ChannelHandlerContext ctx,
            ByteBuf buffer) {
        final MqttVersion mqttVersion = NoodleMqttCodecUtil.getMqttVersion(ctx);
        ...
        return new NoodleMqttDecoder.Result<MqttConnAckVariableHeader>(mqttConnAckVariableHeader, numberOfBytesConsumed);
    }

    private static NoodleMqttDecoder.Result<MqttMessageIdAndPropertiesVariableHeader> decodeMessageIdAndPropertiesVariableHeader(
            ChannelHandlerContext ctx,
            ByteBuf buffer) {
        final MqttVersion mqttVersion = NoodleMqttCodecUtil.getMqttVersion(ctx);
        ...
        if (mqttVersion == MqttVersion.MQTT_5) {
            final NoodleMqttDecoder.Result<MqttProperties> properties = decodeProperties(buffer);
        ...
        return new NoodleMqttDecoder.Result<MqttMessageIdAndPropertiesVariableHeader>(mqttVariableHeader,
                2 + mqtt5Consumed);
    }

    private NoodleMqttDecoder.Result<MqttPubReplyMessageVariableHeader> decodePubReplyMessage(ByteBuf buffer) {
        final int packetId = decodeMessageId(buffer);
        ...
        return new NoodleMqttDecoder.Result<MqttPubReplyMessageVariableHeader>(mqttPubAckVariableHeader, consumed);
    }

    private NoodleMqttDecoder.Result<MqttReasonCodeAndPropertiesVariableHeader> decodeReasonCodeAndPropertiesVariableHeader(
            ByteBuf buffer) {
        ...
        return new NoodleMqttDecoder.Result<MqttReasonCodeAndPropertiesVariableHeader>(
                mqttReasonAndPropsVariableHeader,
                consumed);
    }

    private NoodleMqttDecoder.Result<MqttPublishVariableHeader> decodePublishVariableHeader(
            ChannelHandlerContext ctx,
            ByteBuf buffer,
            MqttFixedHeader mqttFixedHeader) {
        final MqttVersion mqttVersion = NoodleMqttCodecUtil.getMqttVersion(ctx);
        final NoodleMqttDecoder.Result<String> decodedTopic = decodeString(buffer);
        if (!NoodleMqttCodecUtil.isValidPublishTopicName(decodedTopic.value)) {
        ...
        return new NoodleMqttDecoder.Result<MqttPublishVariableHeader>(mqttPublishVariableHeader, numberOfBytesConsumed);
    }
    ...
    private static int decodeMessageId(ByteBuf buffer) {
        ...
        if (!NoodleMqttCodecUtil.isValidMessageId(messageId)) {
     ...
     * @return the payload
     */
    private static NoodleMqttDecoder.Result<?> decodePayload(
            ChannelHandlerContext ctx,
            ByteBuf buffer,
    ...
    private static NoodleMqttDecoder.Result<MqttConnectPayload> decodeConnectionPayload(
            ByteBuf buffer,
            MqttConnectVariableHeader mqttConnectVariableHeader) {
        final NoodleMqttDecoder.Result<String> decodedClientId = decodeString(buffer);
        ...
        if (!NoodleMqttCodecUtil.isValidClientId(mqttVersion, decodedClientIdValue)) {
            throw new MqttIdentifierRejectedException("invalid clientIdentifier: " + decodedClientIdValue);
        }
        int numberOfBytesConsumed = decodedClientId.numberOfBytesConsumed;

        NoodleMqttDecoder.Result<String> decodedWillTopic = null;
        ...
        NoodleMqttDecoder.Result<String> decodedUserName = null;
        ...
        return new NoodleMqttDecoder.Result<MqttConnectPayload>(mqttConnectPayload, numberOfBytesConsumed);
    }

    private static NoodleMqttDecoder.Result<MqttSubscribePayload> decodeSubscribePayload(
            ByteBuf buffer,
            int bytesRemainingInVariablePart) {
        final List<MqttTopicSubscription> subscribeTopics = new ArrayList<MqttTopicSubscription>();
        int numberOfBytesConsumed = 0;
        while (numberOfBytesConsumed < bytesRemainingInVariablePart) {
            final NoodleMqttDecoder.Result<String> decodedTopicName = decodeString(buffer);
        ...
        return new NoodleMqttDecoder.Result<MqttSubscribePayload>(new MqttSubscribePayload(subscribeTopics), numberOfBytesConsumed);
    }

    private static NoodleMqttDecoder.Result<MqttSubAckPayload> decodeSubackPayload(
            ByteBuf buffer,
            int bytesRemainingInVariablePart) {
        ...
        return new NoodleMqttDecoder.Result<MqttSubAckPayload>(new MqttSubAckPayload(grantedQos), numberOfBytesConsumed);
    }

    private static NoodleMqttDecoder.Result<MqttUnsubAckPayload> decodeUnsubAckPayload(
            ChannelHandlerContext ctx,
            ByteBuf buffer,
            int bytesRemainingInVariablePart) {
        ...
        return new NoodleMqttDecoder.Result<MqttUnsubAckPayload>(new MqttUnsubAckPayload(reasonCodes), numberOfBytesConsumed);
    }

    private static NoodleMqttDecoder.Result<MqttUnsubscribePayload> decodeUnsubscribePayload(
            ByteBuf buffer,
            int bytesRemainingInVariablePart) {
        ...
        return new NoodleMqttDecoder.Result<MqttUnsubscribePayload>(
                new MqttUnsubscribePayload(unsubscribeTopics),
                numberOfBytesConsumed);
    }

    private static NoodleMqttDecoder.Result<ByteBuf> decodePublishPayload(ByteBuf buffer, int bytesRemainingInVariablePart) {
        ByteBuf b = buffer.readRetainedSlice(bytesRemainingInVariablePart);
        return new NoodleMqttDecoder.Result<ByteBuf>(b, bytesRemainingInVariablePart);
    }

    private static NoodleMqttDecoder.Result<String> decodeString(ByteBuf buffer) {
        return decodeString(buffer, 0, Integer.MAX_VALUE);
    }

    private static NoodleMqttDecoder.Result<String> decodeString(ByteBuf buffer, int minBytes, int maxBytes) {
        int size = decodeMsbLsb(buffer);
        int numberOfBytesConsumed = 2;
        if (size < minBytes || size > maxBytes) {
            buffer.skipBytes(size);
            numberOfBytesConsumed += size;
            return new NoodleMqttDecoder.Result<String>(null, numberOfBytesConsumed);
        }
        String s = buffer.toString(buffer.readerIndex(), size, CharsetUtil.UTF_8);
        buffer.skipBytes(size);
        numberOfBytesConsumed += size;
        return new NoodleMqttDecoder.Result<String>(s, numberOfBytesConsumed);
    }
    ...
    private static NoodleMqttDecoder.Result<MqttProperties> decodeProperties(ByteBuf buffer) {
        ...
        while (numberOfBytesConsumed < totalPropertiesLength) {
            ...
            switch (propertyType) {
                ...
                case REASON_STRING:
                    final NoodleMqttDecoder.Result<String> stringResult = decodeString(buffer);
                    ...
                    break;
                case USER_PROPERTY:
                    final NoodleMqttDecoder.Result<String> keyResult = decodeString(buffer);
                    final NoodleMqttDecoder.Result<String> valueResult = decodeString(buffer);
        ...
        return new NoodleMqttDecoder.Result<MqttProperties>(decodedProperties, numberOfBytesConsumed);
    }
}
